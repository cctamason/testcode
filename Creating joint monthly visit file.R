...
duplicated(MonthlyAll[,2:5]) # TRUE = rows 41, 238, 

repeats<-MonthlyAll[576:1043,]
write.csv2(repeats,file = "repeats.csv")
View(MonthlyAll[500:576,])
summary(MonthlyAll$hh_id==1)
summary(MonthlyAll$hh_id)
summary(MonthlyAll$day)
summary(MonthlyAll$month)
...
check<-subset(MonthlyAll,as.numeric(cont1.cont1_times)>=25, select=c("hh_id", "cont1.cont1_times", "cont1.cont1_size", "month","day","year","FRA"))
View(check)
